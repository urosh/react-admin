...
*/
/*const addMobileDevice = data => {
	const id = getUserId(data);
	users[id] = Object.assign({}, user, users[id]);
	
	users[id][parametersList.USER_ID] = data[parametersList.USER_ID];
	
	let mobiles = users[id][parametersList.MOBILES].filter(mobile => mobile[parametersList.TOKEN] !== data[parametersList.TOKEN] );
	...
	mobiles.push(data);
	users[id][parametersList.MOBILES] = [...mobiles];
}*/
