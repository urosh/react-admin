...
const setInstrumentFormat = (socket, instrument) => {
	let rooms = [socket[parametersList.LANGUAGE] + '-' + parametersList.INSTRUMENT + '-' + instrument];	
	if(socket[parametersList.TEST_ENABLED]) {
		rooms.push(parametersList.TEST + '-' + socket[parametersList.LANGUAGE] + '-' + parametersList.INSTRUMENT + '-' + instrument)
	}
	return rooms;
}
 ...
 */
 const joinRooms = (socket, rooms) => {
	...
	rooms.forEach(room => {
		joinRooms = joinRooms.concat(setInstrumentFormat(socket, room));
	})
	
	const currentRooms = Object.keys(io.sockets.adapter.rooms).filter(pair => pair.indexOf(parametersList.INSTRUMENT) > -1);
	
	joinRooms = getArrayDifference(joinRooms, currentRooms);
	leaveRooms = getArrayDifference(currentRooms, joinRooms);
