...
const browserTabVisibilityHandler = data => {
	...
	if(pushObject && user[parametersList.MARKET_ALERT_ALLOW]) {

		// Updating socket reference in user's object
	
		pushObject[parametersList.pushActive] = !data[parametersList.TAB_ACTIVE];
		socket[parametersList.SOCKET_ACTIVE] = data[parametersList.TAB_ACTIVE];
		...
		if(user){
			const pairs = data[parametersList.TAB_ACTIVE] ? user[parametersList.PAIRS] : [];
...
const updateMarketAlertSubscription = data => {
	const id = getUserId(data);
	let user = users[id];
	user[parametersList.MARKET_ALERT_ALLOW] = data[parametersList.MARKET_ALERT_ALLOW];
}
module.exports = {
	init,
	checkIfUserExists,
