...
module.exports = {
	...
	registerEvents() {
		this.addEvent('register', config.eventChannels._SOCKETS_, [
			parameterList.MACHINE_HASH,
			parameterList.USER_ID,
			parameterList.TEST_ENABLED,
			parameterList.MARKET_ALERT_ALLOW,
			parameterList.PUSH_ENABLED,
			parameterList.LANGUAGE
		], function(){});
		this.addEvent('connectUser', config.eventChannels._SOCKETS_, [], function(){})
		this.addEvent('connectVisitor', config.eventChannels._SOCKETS_, [], function(){})
	}
