...
module.exports = function(){
	...
	const init = (options, events) => {
		...
		if(!options || !options.redis || !options.redis.pub || !options.redis.sub) {
			return;
		}
		...
		// Get socket events that will use redis
		const socketEvents = events.getEvents('sockets')
			.filter(event => event.distributed);
		
		// Get http events that will use redis redis
		const routesEvents = events.getEvents('http')
			.filter(event => event.method === 'post')
			.filter(event => events.distributed);
		
		// For each event listen to the redis
		let registeredEvents = [].concat(socketEvents, routesEvents);
		
		// Register to the subscribed events
